...
alloc_proc(void) {
     ...
     */
     //LAB5 2012012139 : (update LAB4 steps)
    /*
     * below fields(add in LAB5) in proc_struct need to be initialized
     *       uint32_t wait_state;                        // waiting state
     *       struct proc_struct *cptr, *yptr, *optr;     // relations between processes
	 */
     //LAB6 2012012139 : (update LAB5 steps)
    /*
     ...
     */
    //LAB8:EXERCISE2 2012012139 HINT:need add some code to init fs in proc_struct, ...
...
do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf) {
    ...
    //LAB8:EXERCISE2 2012012139  HINT:how to copy the fs in parent's proc_struct?
    /*
    ...
    */
    ...
    if (copy_files(clone_flags, proc) != 0) {
        cprintf("do_fork-->copy_fs failed!\n");
        goto bad_fork_cleanup_fs;
    }
    if (copy_mm(clone_flags, proc) != 0) {                  //根据clone_flags复制或共享内存信息（本实验中无任何作用）
...
load_icode(int fd, int argc, char **kargv) {
    /* LAB8:EXERCISE2 2012011367  HINT:how to load the file with handler fd  in to process's memory? how to setup argc/argv?
     * MACROs or Functions:
    ...
    //(3.4) find every program section headers
    	off_t phoff = elf->e_phoff + i * sizeof(s_ph);
    	//different from lab7
